<template>
  <div>
    <el-table v-if="tableData.length != 0" :data="tableData">
      ...
      <el-table-column prop="water" label="当前水费" align="center">
      </el-table-column>
...
export default {
  ...
  methods: {
    initData() {
      axios.get("/user/login/readwater?uid=" + this.form.uid).then((res) => {
        if (res.data.code === 200) {
          this.tableData = res.data.data;
          this.option.series[0].data = res.data.data[0].waters.split(",");
    ...
    pay() {
      axios.post("/user/login/paywater", this.form).then((res) => {
        if (res.data.code === 200) {
    ...
    close() {
      axios
        .get("/user/login/updateremwater?uid=" + this.form.uid)
        .then((res) => {
          if (res.data.code === 200) {
            this.$message.success(res.data.msg);
          } else {
    ...
    initmsg() {
      axios.get("/user/login/remandwater?uid=" + this.form.uid).then((res) => {
        if (res.data.code === 200) {
